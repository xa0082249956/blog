...
> \* 我的孩子，在你完成整个游戏之前最好先不要阅读这些内容，因为它们充满了**剧透**。
> \* 当个好孩子，好吗？ 
...
用带十六进制编辑功能的编辑器打开 `data.win`，定位到 `0x725D8C` ( 或许可以 Ctrl + G ) 后将值改为 `01`，保存退出。

### Linux

用带十六进制编辑功能的编辑器打开 `game.unx`，定位到 `0x725DDC` ( 或许可以 Ctrl + G ) 后将值改为 `01`，保存退出。

### macOS

用带十六进制编辑功能的编辑器打开 `game.ios`，定位到 `0x725D8C` ( 或许可以 Mac + G ) 后将值改为 `01`，保存退出。
...
| E                   | 每只眼睛三颗炸。                                 |
| R                   | 每只眼睛七颗炸。                                 |
| I                   | 让 Flowey 笑并重新开始它的战斗。可以在灵魂战斗中使用。          |
| A                   | 切换火焰喷射器。                                 |
...
| V                   | 一次藤蔓攻击。                                  |
| N                   | 一次炸弹攻击。                                  |
| M                   | 一次牙齿上的球攻击。*Ball of teeth/dentata attack.* |
| Shift               | 关闭灵魂指示器，一次一个。                            |
